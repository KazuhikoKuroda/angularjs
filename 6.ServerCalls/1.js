angular.module("mainModule", [])
	.controller("mainController", function($scope, $http, jsonFilter) {
		...
		$scope.simpleGetCall = function(withSuccess) {
			var callURL = (withSuccess ? "1.php" : "invalid-url.php");
		...
		$scope.getCallJSON = function() {
			...
			$http.get("1.php", config)
				.success(function(data, status, headers, config) {
					...
					// $httpサービスさんは、戻り値がJsonっぽいならContent-typeとかおかまいなしに、
					// オブジェクト型につっこむ、と。
					var jsonstring = jsonFilter(data);

					$scope.getCallJSONResult = logResult("GET SUCCESS", jsonstring, status, headers, config);
				})
				.error(function(data, status, headers, config) {
		...
		$scope.getCallTransformFuncs = function() {
			...
			var customTransformResponseFunc1 = function(data, headersGetter) {
				return data + "\nReceived from server " + headersGetter("host");
			};
		...
		$scope.postCall = function() {
			...
			$http.post("1.php", $scope.postData, config)
				.success(function(data, status, headers, config) {
		...
		$scope.putCall = function() {
			...
			$http.put("1.php", $scope.putData, config)
				.success(function(data, status, headers, config) {
